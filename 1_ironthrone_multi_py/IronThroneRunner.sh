...
#
# Requirements:
#   - ~/miniforge3/envs/ase/bin/python
#   - IronThrone-GoT.py Python script
...
run_ironthrone_parallel() {
    ...
    local joblog="${commands_dir}/parallel_joblog.tsv"
    local retry_commands_file="${commands_dir}/parallel_retry_commands.txt"
    local retry_joblog="${commands_dir}/parallel_retry_joblog.tsv"
    > "$commands_file"
    ...
    for i in "$split_dir"/split.R1*.fastq; do
        ...
        echo "${python_bin} ${ironthrone_dir}/IronThrone-GoT.py -r ${run} -f1 ${r1} -f2 ${r2} -c ${config} \
    ...
    rm -f "$joblog" "$retry_commands_file" "$retry_joblog"

    local parallel_status=0
    parallel --joblog "$joblog" -j "$jobs" :::: "$commands_file" || parallel_status=$?

    if [ "$parallel_status" -eq 0 ]; then
        return 0
    fi

    echo "Parallel IronThrone run reported $parallel_status failed split job(s)." >&2

    mapfile -t failed_commands < <(awk -F '\t' 'NR > 1 && $7 != 0 {print $9}' "$joblog")
    if [ "${#failed_commands[@]}" -eq 0 ]; then
        echo "No failed commands were captured in $joblog." >&2
        return "$parallel_status"
    fi

    printf '%s\n' "${failed_commands[@]}" > "$retry_commands_file"
    echo "Retrying ${#failed_commands[@]} failed split job(s) serially..."

    for cmd in "${failed_commands[@]}"; do
        local out_subdir
        out_subdir=$(printf '%s\n' "$cmd" | sed -n 's/.* -o \([^ ]*\).*/\1/p')
        if [ -n "$out_subdir" ] && [ -d "$out_subdir" ]; then
            rm -rf "$out_subdir"
            mkdir -p "$out_subdir"
        fi
    done

    parallel --joblog "$retry_joblog" -j 1 :::: "$retry_commands_file"
}
...
"$python_bin" ${ironthrone_dir}/Combine_IronThrone_Parallel_Output.py "$result_dir" "$sample" \
