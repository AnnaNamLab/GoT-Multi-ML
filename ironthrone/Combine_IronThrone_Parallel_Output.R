...
wd <- options[1]
prefix <- options[2]
pcr_thresh <- as.numeric(options[3])
ld <- as.numeric(options[4])
dupcut <- as.numeric(options[5])
threads <- as.numeric(options[6])
...
for (i in list.files()) {
  split_got <- rbind(split_got, read.delim(paste0(i, "/", prefix, ".summTable.txt"), stringsAsFactors = FALSE))
}
...
write.table(concat_got_df, file = paste0("../", prefix, ".summTable.concat.txt"), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
...
UMI_collapse <- function(GoT_table) {
  GoT_table_to_collapse <- data.frame(
    BC = GoT_table[, "BC"],
    UMI = GoT_table[, "UMI"],
...
# Save output
write.table(max_collapsed_GoT_table, file = paste0("../", prefix, ".summTable.concat.umi_collapsed.txt"), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE)
