...
from .model_config import MODEL_CONFIGS, get_random_hyperparameters
...
def train_classification_model(
    ...
    # Encode categorical features locally so ML input can include both
    # numeric and string metadata columns from GEX/target_info.
    categorical_columns = X_train.select_dtypes(include=["object", "category"]).columns.tolist()
    if categorical_columns:
        logging(
            f"Encoding categorical feature columns with one-hot dummies: {categorical_columns}",
            resultdir,
            "log.txt",
        )
        ...
        X_train = combined.iloc[:train_rows].copy()
        X_test = combined.iloc[train_rows:].copy()
    ...
    if remaining_non_numeric:
        raise ValueError(
            "Non-numeric feature columns remain after encoding: "
    ...
    for model_name in models_to_run:
        if model_name not in MODEL_CONFIGS:
            print(f"Warning: {model_name} is not available and will be skipped")
            continue
    ...
    if not models_params:
        raise ValueError("No supported models are available to train.")
